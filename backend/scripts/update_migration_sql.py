#!/usr/bin/env python3
"""Update content_engine_migration.sql with rich scene structure data."""
...
def generate_rich_scene_structures():
    """Generate all rich scene structures."""
    return {
        "myth_buster": [
            {
                "title": "Hook (belief)",
...
def update_migration_file():
    """Update the migration SQL file with rich scene structures."""
    migration_file = Path("../app/db/content_engine_migration.sql")
    ...
    # Get rich scene structures
    rich_structures = generate_rich_scene_structures()
    
    # Read current content
    with open(migration_file, 'r') as f:
        sql_content = f.read()
    
    # Find and replace each INSERT statement
    for slug, scenes in rich_structures.items():
        # Convert to properly escaped JSON string
        scene_json = json.dumps(scenes).replace("'", "''")
        
        # Find the existing scene_structure for this slug
        pattern = rf"(\(0, '{slug}'.*?scene_structure[\s\n]*=[\s\n]*)'[^']*(\[.*?\])[^']*'(::jsonb\))"
        
        # Replace with new structure
        replacement = f"\\g<1>'{scene_json}'\\g<3>"
        sql_content = re.sub(pattern, replacement, sql_content, flags=re.DOTALL)
...
if __name__ == "__main__":
    print("🔄 Updating content_engine_migration.sql with rich scene structures...")
    success = update_migration_file()
    if success:
        print("\n✅ Migration file updated successfully!")
    else:
        print("\n❌ Failed to update migration file!")