#!/usr/bin/env python3
"""Update video_formats scene_structure with rich storyboard data.
    ...
    try:
        async with crm_session() as db:
            ...
            for slug, scene_structure in formats_data.items():
                ...
                await db.execute(
                    text("""
                        ...
                        SET scene_structure = :structure 
                        WHERE slug = :slug AND org_id = 0
                    """), 
                    {
                        "slug": slug, 
                        "structure": json.dumps(scene_structure)
                    }
                )
            ...
            result = await db.execute(
                text("SELECT slug, jsonb_array_length(scene_structure) as scene_count FROM crm.video_formats WHERE org_id = 0 ORDER BY slug")
            )
